<template>
  <div
    class="memo-container dashboard-item-container display-flex display-column"
    :title="$t('memo.memo')"
  ...
  <self-dialog v-model:show="memoDialogVisible">
            ...
            >
              ...
              <div class="operation display-flex flex-align display-column" @click.stop="">
                <el-popconfirm
                  ...
                  hide-icon
                  :hide-after="50"
                  @confirm="handleDeleteMemo(item.id, item)"
                >
                  <template #reference>
...
const memoDB: Table<{ memo: string; list: string }> = db.table('memoList')
...
const getMemo = async () => {
  const res = await memoDB.toArray()
  const ascending = localStorage.getItem(ascendingEncrypt)
  if (res.length > 0) {
    memoList.value = JSON.parse(decrypt(res[0].list)) as MemoLIst[]
  }
  if (ascending) {
...
const updateMemo = () => {
  memoDB.put({
    memo: 'memo',
    // list: JSON.parse(JSON.stringify(memoList.value))
    list: encrypt(JSON.stringify(memoList.value))
  })
}
...
const handleDeleteMemo = (id: string, item: MemoLIst) => {
...
const handleTopMemo = (index: number) => {
  if (memoList.value[index].isNail && memoList.value[index].notification) {
